...
class SettingsFragment : Fragment(R.layout.fragment_settings) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            val username = viewModel.authUsername.first()
            binding.etUsername.setText(username)
        }

        lifecycleScope.launch {
            val password = viewModel.authPassword.first()
        ...
        binding.btnSaveCredentials.setOnClickListener {
            ...
            // Cache credentials agar tidak perlu read DataStore lagi saat request
            ApiClient.updateCredentials(username, password)
            binding.tvCredentialsStatus.text = "✅ Kredensial disimpan!"
            binding.tvCredentialsStatus.visibility = View.VISIBLE
        }
